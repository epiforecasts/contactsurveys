...
- rate:
  ...
  governing how a download that failed for a reason a retry can fix is
  retried: an incomplete or corrupted download, or a Zenodo request that
  failed with a server error or a rate limit. Any other failure is
  reported as it happened, without retrying. Defaults to an exponential
  backoff of 5 seconds (up to 4 attempts: 1 initial + 3 retries) changed
  by specifying your own rate object, see `?purrr::rate_backoff()` for
  details.
...
#> ✔ Downloading survey list from zenodo [8.2s]
#> 
#> Key: <date_added>
#>     date_added
#>         <char>
...
#> ℹ Files downloaded at '/tmp/RtmpEg537q/zenodo.1095664'.
#> [zen4R][INFO] Files downloaded at '/tmp/RtmpEg537q/zenodo.1095664'.
